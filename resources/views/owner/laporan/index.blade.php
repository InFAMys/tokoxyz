    ...
    <div class="main-content">
        ...
        <div class="card-pink p-3 mb-4 d-print-none">
            <form method="GET" class="d-flex flex-wrap align-items-center gap-2">
                <input type="date" name="from" class="form-control form-control-pink date-filter-input"
                    value="{{ $from->format('Y-m-d') }}">
                <span class="fw-semibold text-muted small">s/d</span>
                <input type="date" name="to" class="form-control form-control-pink date-filter-input"
                        ...
                        <table class="table table-pink table-borderless mobile-card-table mb-0">
                            ...
                            <tbody>
                                @forelse ($topProducts as $p)
                                    <tr>
                                        <td data-label="Produk">{{ $p->nama_barang }}</td>
                                        <td data-label="Qty">{{ $p->qty }}</td>
                                        <td data-label="Pendapatan">Rp {{ number_format($p->revenue, 0, ',', '.') }}</td>
                                    </tr>
                                @empty
                                    <tr>
                        ...
                        <table class="table table-pink table-borderless mobile-card-table mb-0">
                            ...
                            <tbody>
                                @forelse ($categories as $c)
                                    <tr>
                                        <td data-label="Kategori">{{ $c->nama_kategori }}</td>
                                        <td data-label="Qty">{{ $c->qty }}</td>
                                        <td data-label="Pendapatan">Rp {{ number_format($c->revenue, 0, ',', '.') }}</td>
                                    </tr>
                                @empty
                                    <tr>
                                        <td colspan="3" class="text-center py-3 text-muted">Tidak ada data.</td>
                    ...
                    <tbody>
                        @forelse ($sales as $s)
                            <tr>
                                ...
                                <td data-label="Subtotal">Rp {{ number_format($s->subtotal, 0, ',', '.') }}</td>
                                <td data-label="Diskon">- Rp {{ number_format($s->diskon_nominal, 0, ',', '.') }}</td>
                                <td data-label="Ongkir">Rp {{ number_format($s->shipping_cost, 0, ',', '.') }}</td>
                                <td data-label="Total">Rp {{ number_format($s->total_amount, 0, ',', '.') }}</td>
                                <td data-label="Tipe" class="print-hide">
                                ...
                                <td data-label="Status" class="print-hide">
                                    <span class="badge rounded-pill text-bg-{{ $s->statusColor() }}">
                                        {{ $s->statusLabel() }}
                                    </span>
                                </td>
                                <td data-label="Kritik & Saran" class="print-hide">{{ $s->kritik_saran ?? '-' }}</td>
                            </tr>
                        @empty
                            <tr>
